{pkgs, ...}: {
  ...
  #	2. read the `NIX_LD_LIBRARY_PATH` environment variable and use it to set the `LD_LIBRARIES_PATH` environment variable
  #	   for the actual link loader
  #
  # nix-ld's nixos nixos module will set default values for `NIX_LD` and `NIX_LD_LIBRARY_PATH` environment variables, so
  # it can work out of the box:
  #
  #	- https://github.com/NixOS/nixpkgs/blob/nixos-24.11/nixos/modules/programs/nix-ld.nix#L34-L40
  #
